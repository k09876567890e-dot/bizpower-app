...
const App: React.FC = () => {
  ...
  const renderFooter = () => (
    <footer className="bg-slate-50 text-slate-400 py-16 border-t border-slate-200 mt-auto">
      <div className="max-w-4xl mx-auto px-4 text-center">
        ...
        <div className="text-[10px] uppercase tracking-[0.3em] opacity-40">&copy; 2025 BIZPOWER PROJECT.</div>
      </div>
    </footer>
  );
  ...
  const renderHome = () => (
    <div className="animate-fade-in bg-white min-h-screen text-slate-900 relative font-sans overflow-hidden">
      <section className="relative z-10 py-24 sm:py-40 px-4 text-center">
        ...
        <button onClick={startDiagnosis} className="bg-indigo-600 text-white font-black py-5 px-16 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all hover:scale-105 flex items-center gap-3 mx-auto text-xl">
        ...
        <div className="max-w-2xl w-full mx-auto px-4 py-12 flex-grow flex flex-col justify-center relative">
          ...
          <div className="flex flex-col gap-4">
            {question.options.map((option, idx) => (
              <button key={idx} disabled={isTransitioning} onClick={() => handleAnswer(question.id, option)} className="w-full text-left p-6 rounded-2xl bg-white shadow-sm hover:shadow-md hover:border-indigo-600 border-2 border-transparent hover:bg-indigo-50 transition-all duration-200 group flex justify-between items-center">
             ...
             <div className="bg-white rounded-[3rem] p-0 mb-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="w-full aspect-square bg-slate-100 flex items-center justify-center relative overflow-hidden">
                  {!imageLoaded && !imageError && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center"><Loader2 className="animate-spin text-slate-300 mb-2" size={32} /></div>
                  )}
                  {imageError ? (
                    <div className={`absolute inset-0 bg-gradient-to-br ${result.colorTheme} flex flex-col items-center justify-center text-white p-12`}>
                      <div className="bg-white/20 p-8 rounded-[2.5rem] backdrop-blur-md mb-6">{getFallbackIcon(result.type)}</div>
                    </div>
                  ) : (
                    <img src={CHARACTER_IMAGES[result.type] || result.imagePath} alt={result.characterName} onLoad={() => setImageLoaded(true)} onError={() => setImageError(true)} className={`w-full h-full object-cover transition-opacity duration-700 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} />
                  )}
                ...
                <div className="px-10 pb-14 pt-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-inner">
                      <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1.5">Rank</div>
                      <div className="text-6xl font-black text-indigo-600 leading-none">{result.rank}</div>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-inner">
                      <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1.5">Power</div>
        ...
        <div className="max-w-4xl mx-auto px-4 -mt-16 relative z-20 space-y-12">
          <div className="bg-indigo-600 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="text-2xl font-black mb-8 flex items-center gap-4"><Sparkles size={28} className="text-amber-300 fill-amber-300" /> AI 戦略分析レポート</h3>
              {loadingContent ? (
                ...
                <div className="bg-white/10 rounded-3xl p-8 border border-white/20 backdrop-blur-sm shadow-inner">
                  <p className="text-white leading-relaxed text-lg font-bold whitespace-pre-wrap">{aiAdvice || "解析できませんでした。"}</p>
                </div>
              )}
            </div>
          </div>
          <div className="bg-white border border-slate-200 rounded-[3rem] p-10 sm:p-14 shadow-xl">
             <div className="flex flex-col lg:flex-row gap-16 items-center">
                ...
                <div className="w-full lg:w-1/2">
                  ...
                  <p className="text-slate-600 leading-[1.8] text-lg font-bold text-justify">{result.description}</p>
                </div>
             </div>
          </div>
               ...
               {result.recommendedActions.map((action, i) => (
                  <div key={i} className="bg-white border border-slate-100 p-10 rounded-[2.5rem] flex flex-col items-center justify-between hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group">
                    <div className="w-full text-center">
                      <div className="mb-8 flex justify-center"><div className="p-5 bg-indigo-50 rounded-[1.5rem] group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300"><CheckCircle size={32} /></div></div>
                      <h4 className="font-black text-slate-900 mb-5 text-xl">{action.label}</h4>
                      <p className="text-sm text-slate-500 leading-relaxed font-bold mb-10 min-h-[3rem]">{action.description}</p>
                    </div>
                    <a href={action.url} target="_blank" rel="noopener noreferrer" className="w-full inline-flex justify-center items-center gap-3 bg-indigo-600 text-white text-sm font-black px-8 py-5 rounded-2xl hover:bg-indigo-700 transition-all shadow-lg active:scale-95">詳細を調査する <ExternalLink size={18} /></a>
                  </div>
               ))}
             </div>
          </div>
  ...
  const renderTextPage = (title: string, content: string) => (
    <div className="bg-slate-50 min-h-screen py-24 px-4 animate-fade-in">
      <div className="max-w-3xl mx-auto bg-white p-10 sm:p-20 rounded-[3rem] shadow-2xl border border-slate-100">
        <h2 className="text-4xl font-black text-slate-900 mb-12 border-b border-slate-100 pb-8">{title}</h2>
        <div className="text-slate-600 leading-relaxed font-bold whitespace-pre-wrap text-lg text-justify">{content}</div>
